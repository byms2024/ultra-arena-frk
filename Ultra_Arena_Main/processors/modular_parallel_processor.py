...
from processors.processing_strategies import ProcessingStrategyFactory
...
class ModularParallelProcessor:
    ...
    def _group_files_parallel(self, pdf_files: List[str]) -> List[List[str]]:
        ...
        max_files_per_group = self.config.get("max_num_file_parts_per_row_jsonl", 8)
        groups = []
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            ...
            for future in as_completed(future_to_group):
                ...
                try:
                    ...
                    # Update overall stats to match backup project exactly
    ...
    def _calculate_final_statistics(self, start_time: float):
        ...
        self.structured_output['overall_stats']['total_wall_time_in_sec'] = int(time.time() - start_time)
        ...
        for file_path, result in self.structured_output['file_stats'].items():
            if isinstance(result, dict):
                ...
                if (file_process_result.get('success') == False and 
                    file_process_result.get('retry_round', 0) >= self.config.get('num_retry_for_mandatory_keys', 2)):
                    failed_count += 1
